...
class ContextualCritic(nn.Module):
  ...
  def forward(self, x, f_obj_to_img = None, get_ave = False):
    x = self.critic_net(x)
    x = x.view(x.shape[0], -1)

    if f_obj_to_img is not None:
      if (x.shape[0] > f_obj_to_img.shape[0]):
        return self.ave_split(x, f_obj_to_img)
    if get_ave:
  ...
  def ave_imgs(self, x, f_obj_to_img):
    ...
    for i in range(max(f_obj_to_img) + 1):
      ...
      fake_mean = torch.mean(x[img_inds], dim = 0)
      fake_avgs.append(fake_mean)
    
    ave_x = torch.cat(fake_avgs)
    
    return ave_x

  def ave_split(self, x, f_obj_to_img):
    """
    # Get average scores of all objects per image
    """
    fake_avgs = []
    ...
    for i in range(max(f_obj_to_img) + 1):
      ...
      fake_mean = torch.mean(fake_scores[img_inds], dim = 0)
      real_mean = torch.mean(real_scores[img_inds], dim = 0)
      fake_avgs.append(fake_mean)
