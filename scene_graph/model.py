...
from scene_graph.graph import GraphTripleConv, GraphTripleConvNet
from scene_graph.crn import RefinementNetwork
from scene_graph.layout import boxes_to_layout, masks_to_layout
from scene_graph.layers import build_mlp
...
class Sg2ImModel(nn.Module):
  def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
               ...
               refinement_network_dims=(1024, 512, 256, 128, 64),
    ...
    # Input:
    # - Object embedding output of GConv: shape => O x DOut
    # Layers:
    # - (1) Linear: transforms Dout dimensions to H -> ReLU
    # - (2) Linear: transforms H to 4 for the bounding box
    """
    box_net_dim = 4
    box_net_layers = [gconv_dim, gconv_hidden_dim, box_net_dim]
    self.box_net = build_mlp(box_net_layers, batch_norm=mlp_normalization)

    # Build Mask Regression Network
    self.mask_net = None
    ...
    refinement_kwargs = {
      'dims': (gconv_dim + layout_noise_dim,) + refinement_network_dims,
  ...
  def _build_mask_net(self, num_objs, dim, mask_size):
    ...
    # Input:
    # - Reshaped object embedding output of GConv: shape => O x DOut x 1 x 1
    # Layers:
    # - while current size < mask_size
    #  * Nearest neighbor upsample (scale = 2)
    #  * Batch Norm
    #  * Conv: k = 3x3; s = 1; p = 1; in_ch = DOut; out_ch = DOut
    #  * ReLU 
    # - Conv: k = 1x1; s = 1; p = 0; in_ch = DOut; out_ch = 1
    # - Sigmoid
    """
    output_dim = 1
    layers, cur_size = [], 1
  ...
  def forward(self, objs, triples, obj_to_img=None,
    ...
    """
    # Split the Triples to subject, predicate, object
    """
    s, p, o = triples.chunk(3, dim=1)           # All have shape (T, 1)
    s, p, o = [x.squeeze(1) for x in [s, p, o]] # Now have shape (T,)
    edges = torch.stack([s, o], dim=1)          # Shape is (T, 2)
    # print("DIS EDGES: " + str(edges.shape))

    if obj_to_img is None:
      obj_to_img = torch.zeros(O, dtype=objs.dtype, device=objs.device)

    """
    # Embed ids of objects and predicates
    """
    ...
    # get bounding boxes of objects with relationship
  ...
  def encode_scene_graphs(self, scene_graphs):
    ...
    # print("objs hehe: " + str(objs.shape))
    # print("triples hehe: " + str(triples.shape))
    # print("obj_to_img hehe: " + str(obj_to_img.shape))
  ...
  def forward_json(self, scene_graphs):
    ...
    # print("MAH IMAGE SIZE = " + str(self.image_size))
    # print("HEHE refine: " + str(self.refinement_net))
