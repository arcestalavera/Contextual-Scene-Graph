...
from scene_graph.layers import get_normalization_2d, get_activation
...
class RefinementModule(nn.Module):
  def __init__(self, layout_dim, input_dim, output_dim,
    ...
    # Input:
    # - Scene Layout from Box and Mask Regression Network
    # - Gaussian Noise for the first Module
    # Pre-process:
    # - Average Pooling (layout)
    # - 2x Scale Upsample (input)
    # - Concat Layout and Input
    # Layers:
    # (1) 3x3 Conv (p = 1, s = 1) [(layout + input) dim -> output dim] -> batch norm -> leaky relu
    # (2) 3x3 conv (p = 1, s = 1) [output dim -> output dim] -> batch norm -> leaky relu
    """
    layers = []
  ...
  def forward(self, layout, feats):
    ...
    if HH > H:
      ...
      # pool the layout to match size of features
      layout = F.avg_pool2d(layout, kernel_size=factor, stride=factor)

    # concatenate the layout and features on channels
